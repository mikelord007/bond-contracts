...
contract BondFixedExpiryTeller is BondBaseTeller, IBondFixedExpiryTeller {
    ...
    ) BondBaseTeller(protocol_, aggregator_, treasury_, votingEscrow_, guardian_, authority_) {
    ...
    function _handlePayout(
        ...
        if (vesting_ > uint48(block.timestamp)) {
            ...
            if(!(address(underlying_) == address(0))) {
                ...
                (bool sent, ) = recipient_.call{value: payout_}("");
    ...
    function create(
        ...
        uint48 expiry_,
        uint256 amount_
    ) external override payable nonReentrant returns (ERC20BondToken, uint256) {
    ...
    function redeem(ERC20BondToken token_, uint256 amount_) external override nonReentrant {
        ...
        _giveOutBonus(msg.sender, amount_);
